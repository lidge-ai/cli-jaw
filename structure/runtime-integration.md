...
`user_stop` / `steer_kill`) is the display discriminator on `orchestrate_done`.
The collector must not invent a cause: a missing or unknown value keeps
`tg.stopped`. Do not split `runtimeStatus`.
